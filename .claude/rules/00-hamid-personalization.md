...
- **قانون گزارش ساده (دستور حمید، ۲۵ اوت — «اصلاً متوجه حرفت نمی‌شوم»)**:
  هیچ گزارشی با اصطلاح داخلی (پاسبان، PR، عیب کلاس، merge، sentinel…)
  بدون ترجمهٔ فارسی ساده نوشته نمی‌شود. هر گزارش خرابی سه جمله دارد:
  «چی خراب شده بود» → «چی کار کردیم» → «از این به بعد چه انتظاری داشته
  باش». جزئیات فنی فقط بعد از این سه جمله، و فقط اگر لازم بود.
