...
## شناسنامهٔ تحویل ۱۳ سپتامبر (دستور حمید: «بهترین کد پایتون رو در اختیار داشبورد بذار»)

  · **چه چیزی تحویل می‌شود**: همین فایل (منطق دست‌نخورده از ۳.۱) —
    استراتژی اصلیِ ارسال `ibsPullback` + دروازه‌های روند/بستر/کارمزد/
    محافظ لیکویید + مارجین ایزوله + استاپ/تارگت اجباری. نسخهٔ فشرده
    برای جعبهٔ داشبورد: `ghoghnoos.py` (بازساخته از همین کد).
  · **چرا «بهترین» همین است، با عدد**: بک‌تست ۸۰ نماد × ۱۵۸ سری روی
    کندل واقعی (۱۳ سپتامبر ۱۴:۱۱ UTC)، سنجهٔ سختگیرانه (خروجِ سربه‌سر = ۰،
    خالص از کارمزد ۰.۱۵٪)، بوت‌استرپ دو-خوشه‌ای (نماد × روز) و شیداک روی
    ۲۱ خانه: `ibs|overall` n=۴۱٬۸۱۱ → **+۰.۰۶۴R**، CI [+۰.۰۱۹, +۰.۱۰۷] —
    تنها بازو/برشی که از همهٔ سخت‌گیری‌ها بالای صفر ماند. جعبهٔ ×۲/×۳
    (هندسهٔ بزرگ‌تر) با ۱۲۸ روز داده اثبات نشد (UNDECIDED)؛ پس هندسه
    **عوض نشده**. `base`/`widebuf` overall و short: REJECT.
  · **شورت**: در هیچ هندسه‌ای بالای صفر نشد؛ در `ibs` نزدیک صفر.
    داشبورد شورت را با همان دروازه‌ها می‌گیرد؛ کم‌کردنِ سهمش تصمیمِ حمید
    است، نه این فایل.
  · **پیش‌نیازهای آستانهٔ پول واقعی (CLAUDE.md)، وضعیت امروز**:
      ۱ دروازهٔ کارمزد پیش از پوزیشن — ✅ در همین فایل (`fee_r`، دام اسکالپ).
      ۲ فهرست ارزهای صفر-کارمزد — ⏳ UNVERIFIED (config/fees.json).
      ۳ کیل‌سوییچ چندماشه (سقف ضرر تجمعی، نرخ سفارش، انحراف قیمت، dead-man،
        ناهمخوانی صرافی) — ✅ ساخته و سیم‌کشی: hamid/killswitch.py، دروازهٔ
        اجرا (execution_gate.guard) و رابط داشبورد (liam9_connector) قبل از هر
        سفارش guard() را می‌زنند؛ ۱۳ آزمون در hamid/test_risk_guards.py.
        تصحیح ۱۴ سپتامبر: نسخهٔ ۱۳ سپتامبر همین خط «❌ نساخته» گفته بود و غلط
        بود — کیل‌سوییچ از ۱۶ اوت هست. ⚠️ هنوز روی سفارش واقعی تمرین نشده
        (LIVE_EXECUTION=false)؛ ریست فقط دستی با تأیید حمید.
      ۴ سقف سخت سایز — ✅ ۲۵–۳۰٪ مارجین، ≤۳ پوزیشن، محافظ لیکویید (بند ۲.۶).
      ۵ گذار مرحله‌ای پیپر→لایو — ◐ عدد پیپر سقف خوش‌بینانه است؛ لبهٔ خالص
        فقط +۰.۰۶۴R است — یعنی کوچک و شکننده.
      ۶ Reconciliation پوزیشن‌های واقعی — ◐ فقط از فایلِ صادرشده
        (`hamid/live_results.py`)، نه از API.
  · **مرز**: `LIVE_EXECUTION` در این ریپو `false` می‌ماند (قانون ۰۵/۱۰ و
    قانون ۱۸ بند ۳). روشن‌کردنِ اجرای زنده کارِ داشبورد/حمید است و با
    وضعیت بالا — به‌ویژه بندهای ۳ و ۵ — تصمیمی است که این فایل برایش
    نمی‌گیرد. اهرم بالای ۱۵ فقط با تأیید صریح (بند ۲.۶ = همان تأیید).
...
## تازه‌های ۳.۰ (دستور حمید، ۲۹ اوت)
  ...
  · **نسخهٔ فشردهٔ داشبورد** — این فایل با هر نسخه بزرگ‌تر شد و در
    ...
    همین کد را بدون کامنت/داک‌استرینگ در `ghoghnoos.py`
