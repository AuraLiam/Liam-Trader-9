...
def run():
    ...
    # ۱۱ سپتامبر: E13 (آنالوگ تاریخی) با مطالعهٔ یک‌سالهٔ رویداد
    # (signals/event-study.json) ردپا گرفت — شش شد پنج.
    # این مجموعه فقط حق دارد کوچک شود؛ انجین تازه‌ای که بی‌ردپا شود سرخ
    # می‌کند، و برگشتِ یکی از این‌ها به فهرست هم سرخ می‌کند.
    # ۱۵ سپتامبر: E05 با موضع بازار (signals/market-stance.json) ردپا گرفت —
    # پنج شد چهار — و همین بررسی که با «==» می‌سنجید، بهبود را خرابی دید و
    # زنجیره را ۶ ساعت سرخ نگه داشت (درس ۶ سپتامبر: آزمون خاصیت را بسنجد،
    # نه شکل را). خاصیت: مجموعه فقط کوچک می‌شود — زیرمجموعهٔ آخرین فهرست.
    KNOWN_TRACELESS = {"E04", "E05", "E07", "E09", "E24"}
    check("انجین‌های بی‌ردپای ممیزی فقط کم می‌شوند (زیرمجموعهٔ فهرست شناخته‌شده)",
          tr <= KNOWN_TRACELESS, str(tr - KNOWN_TRACELESS))
