...
def build():
    ...
    L = [
        ...
        *section_exam(),
        ...
        "",
        "## ۵. مهاجرت به پوشهٔ تازه — سریع و کم‌توکن",
        "- `scripts/bootstrap-session.sh [مقصد]` — کلونِ **کم‌عمق** (۵۰ کامیت؛ تاریخچهٔ کامل ۴.۴GB است و هر fetch/push را دقیقه‌ها کند می‌کند)، نصب محیط یگانه، و چاپِ حکم سامانه + صف بازبینی. هیچ چیزِ در حال اجرا (Actions، سرویس محلی) به پوشه وابسته نیست؛ مهاجرت روی روند کار اثری ندارد.",
        "- در چتِ تازه **فایل نخوان**؛ سه ابزار MCP بالا کافی‌اند. زیرایجنت فقط برای ردیف‌های `needs_reasoning`. هر عدد از ابزار (tg_audit، geometry_verdict، gate_verdict)، نه از حافظهٔ چت قبلی.",
        "- بک‌آپ قبل از هر دستهٔ تغییر: `git push origin main:refs/heads/backup/main-pre-<موضوع>-<تاریخ>` و راستی‌آزمایی با `git ls-remote`.",
        "",
