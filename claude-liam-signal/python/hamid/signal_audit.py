...
def c_repeat_expired(sent, closed_all):
    ...
    **این بررسی دروازه نیست** (قانون ۰۳): فقط می‌شمارد و اقدام پیشنهاد
    ...
    return _finding("ستاپِ تکرارشوندهٔ منقضی", not hits,
                    ...
                    "دروازهٔ ستاپ یخ‌زده از ۵ سپتامبر فعال است "
                    "(telegram.FROZEN_MIN_EXPIRED=2) — ارسالِ باقی‌مانده "
                    "یعنی یا قبل از دروازه رفته یا دروازه دور خورده؛ "
                    "زمانِ ارسال را با تاریخ دروازه بسنج" if hits else
