...
def run():
    ...
    # «ثبت‌شده در فهرست» با «پرشده» یکی نیست: دفتر آزمایش باندی
    # (exp-short) تازه متولد می‌شود و تا اولین تسویه خالی است — خالی‌بودنِ
    # دفترِ نوزاد عیب نیست. فقط دفترهایی که تاریخ دارند باید پر باشند.
    names = {r["_ledger"] for r in sh}
    check("دفترهای تاریخ‌دار همه در استخر هستند",
          names >= {"ارسالی", "وتوشده", "تمرین", "پولبک ۱"}, str(names))
    check("دفتر آزمایش باندی در فهرست ثبت است (پر یا خالی)",
          any(p == "exp-short" for p, _ in L.LEDGERS), str(L.LEDGERS))
