...
۵. **چهار مراقب روی این داده اصلاً غربال نمی‌کنند** (کشفِ اجرای ۱۲
   سپتامبر، ۱۲۰ نماد، ۱۷ فوریه تا ۱۱ سپتامبر): ثور، حمل، میزان و سنبله
   دقیقاً یک عدد دادند — n=۳۰۵۳، خالص +۰.۰۱۰۸R، برد ۵۹.۱٪، صفر امتناع،
   صفر ردِ زیرآستانه. علتش بیرونِ آمار است: میدانی که رأیشان به آن
   نگاه می‌کند (`trend1`، `impulse`، `rr`، `candle_src`+`sync`) را
   `enrich` روی **هر** ستاپ پر می‌کند و مقدارش تقریباً همیشه از ‎+۰.۱۵‎
   رد می‌شود. پس میزشان با «همه‌چیز را بخر» یکی است و این اجرا دربارهٔ
   تخصصشان **هیچ** نمی‌گوید — نه خوب، نه بد.

   این عدد را نباید به‌عنوان «کارنامهٔ ثور» گزارش کرد. برای سنجشِ واقعی
   رأیشان باید درجه‌بندی شود (مثلاً ثور فقط وقتی هر دو تایم هم‌جهت‌اند
   تأیید کند، نه وقتی یکی هم‌جهت است)، و آن تغییرِ رأی است نه تغییرِ
   آزمایش — یعنی از مسیر قانون ۰۳ و با دستور حمید.

   آن‌که واقعاً غربال کرد: حوت (۱۰۸۳ ردِ زیرآستانه)، اسد (۵۱۹۸ امتناع)،
   سرطان (۹۱۴۲ ردِ زیرآستانه). و پنج مراقب (عقرب، جوزا، جدی، قوس، دلو)
   ۱۰۰٪ ممتنع ماندند — همان بندِ ۲ بالا، به‌اضافهٔ جدی که کارنامهٔ
   (ارز، جهت) هرگز به کفِ ۱۲ نمونه نرسید.
