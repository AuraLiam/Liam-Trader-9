...
def settle_books(report):
    ...
    # آینهٔ بازوهای تریل، پیش از تسویه (۱ سپتامبر): هر ستاپِ باز با دو
    # نردبانِ دیگر هم باز می‌شود تا A/B **جفتی** روی همان ستاپ ساخته
    # شود. فقط دفتر پیپر؛ نه ستاپی می‌سازد، نه چیزی به تلگرام می‌رود.
    try:
    ...
    try:
        ...
        back = _mem2.digest_backlog(skip={paper.trade_key(t) for t in newly})
        if fed or back:
