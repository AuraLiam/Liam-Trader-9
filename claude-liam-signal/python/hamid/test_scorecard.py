...
def run():
    ...
    # اصلاح ۱ سپتامبر: این بررسی عددها را در **متنِ سورس** می‌گشت
    # («12 * H») — یعنی همان قراردادِ کهنه را تضمین می‌کرد. ولی حمید
    # ۲۷ اوت پنجره را ۶ ساعت کرد؛ نتیجه‌اش ۴ تخلفِ کاذب روی کارنامهٔ
    # E25 بود. حالا متر ثابت را از خودِ `telegram` می‌خواند و این بررسی
    # همان را تضمین می‌کند، نه یک عددِ ثابت.
    import telegram as _TG
    check("پنجرهٔ متر از خودِ telegram می‌آید، نه عددِ سفت‌نوشته",
          "from telegram import TTL_MS" in src and "12 * H" not in src)
    check("هر سه کلیدِ قرارداد سنجیده می‌شود (بی‌استراتژی · جفت · هم‌استراتژی)",
          "last_any" in src and "last_pair" in src and "last_strat" in src)
    check("سقف هر ارز هم سنجیده می‌شود", ">= 2" in src)
    check("پنجرهٔ حکم تازه است، نه کلِ تاریخ (متری که سبز نشود بی‌فایده است)",
          "RECENT_H" in src)
    check("و پنجرهٔ واقعیِ کد همان است که متر می‌سنجد",
          _TG.TTL_MS / 3_600_000 == 6)
