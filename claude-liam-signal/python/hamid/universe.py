...
# ── ارزهای بلاک‌شده — دستور صریح حمید (۱۶ سپتامبر) ────────────────────────
#
# حمید: «ارز TRX را از چرخهٔ تحلیل و پنل خودمان کلاً بلاک کن، چون در ارسال
# سیگنال این ارز بارها سیگنال می‌شود و هیچ‌وقت هم ارزش ریسک و R به R
# مناسبی نداشت.»
#
# این فهرست **دستورِ مالک** است، نه حکمِ آماری — و همین‌جا صریح گفته
# می‌شود تا بعداً کسی آن را «قاعده‌ای که CI داشت» نخواند. جایش این‌جاست
# چون هر مصرف‌کنندهٔ جهانِ نمادها از همین ماژول می‌خواند؛ فهرستِ دوم در
# فایل دیگر یعنی همان کلاسِ عیبی که امشب در اردر بلاک دیدیم.
BLOCKED = {"TRX"}


def is_blocked(sym):
    """نماد (یا پایه‌اش) در فهرست بلاک است؟ — TRXUSDT، TRX، trxusdt همه."""
    if not sym:
        return False
    s = str(sym).upper()
