...
# ── تطبیق با هویتِ ستاپ، نه فقط پنجرهٔ زمانی (اصلاح ۱ سپتامبر) ──────────
#
# ارسالِ دومِ همان ستاپ (بعد از پنجرهٔ ضدتکرار) ردیف دوم نمی‌سازد — همان
# لیمیتِ منتظر است. با معیارِ زمانیِ تنها، «بی‌ردپا» شمرده می‌شد.
# نمونهٔ واقعی: LOKAUSDT ارسال ۰۰:۱۶ و ردیفِ باز از ۱۵:۱۰ با همان ورودِ
# ۰.۱۲۳۶ — ۵۴۷ دقیقه فاصله، بیرون از پنجرهٔ ۹۰ دقیقه‌ای.
check("تطبیق با ورودِ یکسان مقدم بر پنجرهٔ زمانی است",
      'r.get("entry") == entry' in src
      and src.index('r.get("entry") == entry') < src.index("90 * 60000"))
check("جهت هم در تطبیقِ هویت شرط است (لانگ با شورت یکی نشود)",
      'r.get("dir") == s.get("dir")' in src)
check("پنجرهٔ زمانی به‌عنوان راهِ دوم باقی مانده",
      "90 * 60000" in src)
