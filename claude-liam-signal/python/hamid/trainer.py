...
# ── کفِ استاپ — تغییرِ سنجش‌شدهٔ ۹ سپتامبر (تأیید صریح حمید) ──────────────
#
# کفِ قبلی ۰.۱۵٪ بود. آزمایشگاه هندسه (`hamid/geom_lab.py`، ۹ خانه، ۷ ماه
# کندل واقعی، ۸۱ نماد از ۱۰۰) نشان داد کفِ استاپ **تنها محورِ مؤثر** است و
# محورِ RR به‌تنهایی خالص را بدتر می‌کند:
#
#   rr2.0/stop0.15 (کنترل) n=۶۰۶۳ خالص −۰.۰۸۵۹ [−۰.۱۱۳۸, −۰.۰۵۷۹]
#   rr2.0/stop1.5          n=۳۲۴۷ خالص +۰.۰۰۶۶ [−۰.۰۲۹۹, +۰.۰۴۳۲]
#   اختلاف (بوت‌استرپ خوشه‌ای روی نماد) +۰.۰۹۲۵ [+۰.۰۳۴۱, +۰.۱۵۶۳]
#     → PROMOTE_CANDIDATE در آستانهٔ Šidák (۸ مقایسه، α≈۰.۰۰۶۴)
#
# سازوکار، از مسیرِ دومِ مستقل: کارمزدِ ضمنی (ناخالص−خالص) تقسیم بر میانهٔ
# کارمزد = **۱.۴۲×** روی کفِ ۰.۱۵ و ۰.۹۷× روی کفِ ۱.۵ — یعنی زیانِ کنترل
# دُمِ چاقِ معامله‌های استاپ-ریز است، نه یک سوگیریِ یکنواخت. همان هویتِ
# `سهم کارمزد از R = کارمزد٪ ÷ استاپ٪` که میز اسکلپ ۱د و میز شوک را کشت.
#
# مرزِ صادقانه (روی همان اعداد): این میز را **سودده نمی‌کند** — CI بهترین
# خانه هنوز شاملِ صفر است. و ۴۶٪ نمونه حذف می‌شود (۶۰۶۳→۳۲۴۷)، حذفی که
# تصادفی نیست: نمادهای کم‌نوسان بیرون می‌مانند.
MIN_STOP_PCT = 1.5    # کفِ استاپ٪ — زیر این، سهمِ کارمزد از R بیش از حد است
MAX_STOP_PCT = 6.0    # سقفِ استاپ٪ — بالاتر از این، «ساختار» بی‌معنا می‌شود
...
def decide(window, tf="15m", near_fn=None, rr=None,
    ...
    _lo = MIN_STOP_PCT if min_stop is None else min_stop
    _hi = MAX_STOP_PCT if max_stop is None else max_stop
    ...
    def pack(d, sl, setup, ob=None):
        ...
        why = {"stage": "practice", "trainer": 1, "setup": setup,
               ...
               # ⚠️ نامِ `trend_4h` **گمراه‌کننده است** (کشفِ ۷ سپتامبر):
               # `t` از `trend(window)` می‌آید، یعنی روندِ **همان
               # تایم‌فریمِ ورود**، نه کندل ۴ ساعته. هر کس از روی دفتر
               # تحلیل کند و اسم را باور کند، چیزِ دیگری می‌فهمد — و
               # یک بار همین اتفاق افتاد: موتور شورت نسخهٔ ۱.۰ فیلتر را
               # روی سری ۴س واقعی اجرا کرد و از دفتر جدا افتاد.
               # نام عوض نمی‌شود چون شرط‌های ماشین شبانه و ۱۷ هزار ردیفِ
               # تاریخی به همین کلید بسته‌اند؛ به‌جایش این‌جا ثبت شد و
               ...
               "min_stop": _lo,                       # همان دلیل، برای کف
               # استاپ — و امروز (۹ سپتامبر) لازم‌تر از rr: کفِ تولید از
               # ۰.۱۵ به ۱.۵ رفت، پس خودِ دفترِ تاریخی دو تعریف دارد و
               # بدونِ این فیلد، تفکیکشان فقط با حدسِ تاریخ ممکن بود.
