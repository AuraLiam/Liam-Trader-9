...
# ── سمت داشبورد ─────────────────────────────────────────────────────────
def _macro(**kw):
    # `generated` از ساعتِ **واقعی** می‌آید، نه از NOWِ ثابت. علتش یک
    # خرابیِ اندازه‌گیری‌شده است: `_finalize` عمداً `macro_state()` را بدون
    # زمان صدا می‌زند (در تولید همیشه «حالا» است)، پس با `generated=NOW`
    # این آزمون فقط تا ۱۸۰ دقیقه بعد از همان ثابت سبز می‌ماند و بعد
    # خودبه‌خود سرخ می‌شود — ۱۷ سپتامبر دقیقاً همین شد و از ۰۲:۰۶ UTC
    # دروازهٔ سختِ چرخهٔ حمید را خواباند. آزمونی که به گذرِ زمان حساس
    # باشد، خرابی می‌سازد نه کشف. سنجشِ کهنگی جای خودش با `macro_state(NOW)`
    # و مهرِ صریح انجام می‌شود (بررسی «تقویمِ کهنه‌تر از سقف بی‌اثر است»).
    S.MACRO.clear()
    S.MACRO.update({"in_window": True, "generated": time.time() * 1000, "size_mult": 0.5,
