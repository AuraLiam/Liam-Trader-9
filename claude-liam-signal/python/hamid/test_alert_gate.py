...
check("نگهبان یکپارچگی از دروازه رد می‌شود (عیب دوم ۲۳ اوت)",
      "alert_gate" in sn)
# خاصیت، نه شکل (درس ۶ سپتامبر): کلید از `stale_bucket(len(...))` ساخته
# می‌شود — روی هر فهرستی (از ۸ سپتامبر فقط ارسالی‌ها: `mine_stale`).
check("کلید پاسبان پوزیشن سطلی است، نه فهرست نماد و نه شمارش دقیق",
      re.search(r"key = stale_bucket\(len\(\w+\)\)", pw) is not None,
