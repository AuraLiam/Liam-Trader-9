...
پیام عملی می‌فرستد: «الان استاپ را بیاور به X». سه پله، همان قانون:
    ...
    (بازنویسی ۱۴ سپتامبر به نسخهٔ سه — متن قدیمی ⅓/⅔ در تاریخچهٔ git)
    پلهٔ ۱ — سود از کارمزد گذشت → قفل ۸۰٪ فعال
    پلهٔ ۲ — TP1                → استاپ روی ۸۰٪ مسیر
...
def rungs(entry, tp1, direction):
    """پله‌های اعلام — **نسخهٔ سهٔ قانون تریل** (۶ سپتامبر)، نه نردبان ⅓/⅔ بازنشسته.

    ممیزی تلگرام ۱۴ سپتامبر: ۱۰۷ پیامِ «⅓ مسیر… / ⅔ مسیر…» در ۷ روز، همه
    دستورِ نسخهٔ یک — در حالی که دفتر پیپر از ۶ سپتامبر با نسخهٔ سه بسته
    می‌شود (استاپ روی ۸۰٪ بهترین سود، فقط بعد از گذشتن از کارمزد). یعنی
    اجرای دستی حمید و دفتر از هم جدا افتاده بودند. حالا دو پله، هم‌قرارداد
    با `paper.PROD_TRAIL_FRAC`:
      ۱ — سود از کارمزدِ دو سر گذشت → قفل ۸۰٪ فعال (استاپ = ورود + ۸۰٪ سود)
      ۲ — TP1 → همان قفل روی TP1 (استاپ = ورود + ۸۰٪ فاصلهٔ TP1)
    → [(سطحِ ماشه، استاپِ جدید، متن)]
    """
    try:
        from hamid.paper import PROD_TRAIL_FRAC as frac
    ...
    return [
        (arm,
         entry + d * frac * fee,
         f"سود از کارمزد گذشت — قفل {pct}٪ فعال: از این‌جا استاپ = ورود + {pct}٪ بهترین سود، فقط در جهت سود"),
        (tp1,
         entry + d * frac * dist,
         f"TP1 زده شد — استاپ به {pct}٪ مسیر تا TP1؛ بالاتر که رفت، همان {pct}٪ بهترین سود"),
    ]
