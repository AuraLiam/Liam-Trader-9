...
try:
    ...
    # کندل ۱ تا ۹۹ پایین می‌رود (بدترین نقطه)، بعد بی‌برگشت تا تارگت
    # می‌رود. موضوعِ این بخش ثبتِ mfe/mae روی دفتر سیگنال است، نه قاعدهٔ
    # تریل — پس مسیر طوری چیده شده که **هیچ نسخه‌ای از تریل** وسطش
    # نبندد: کفِ هر کندل بالای تریلِ برآمده از بهترین سودِ تا آن لحظه
    # می‌ماند. (تا ۶ سپتامبر مسیرِ قبلی یک برگشت داشت؛ با تریل نسخهٔ سه
    # — استاپ روی ۸۰٪ بهترین سود — معامله در کندل ۳ بسته می‌شد و همین
    # سه بررسی قرمز شدند و میز تمرین ۵ روز خوابید.)
    bars = [(100.0, 100.2, 99.0), (100.5, 101.5, 100.3),
            (101.5, 103.0, 101.3), (103.0, 104.5, 102.8)]
    # شرطِ صریحِ همان چیدمان، وابسته به خودِ ثابتِ تولید: اگر کسی سهمِ
    # تریل را عوض کرد، این‌جا یک پیامِ روشن می‌گیرد نه سه شکستِ گیج‌کننده.
    _frac, _best, _ok = getattr(paper, "PROD_TRAIL_FRAC", 0.80), 0.0, True
    for _o, _h, _lo in bars:
        if _best > 0 and _lo < 100.0 + _frac * _best:
            _ok = False
        _best = max(_best, _h - 100.0)
    check("چیدمانِ آزمون با سهمِ تریلِ تولید سازگار است", _ok,
          f"PROD_TRAIL_FRAC={_frac} — مسیر باید بی‌برگشت به تارگت برسد")
