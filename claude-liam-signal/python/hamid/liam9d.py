...
JOBS = [
    ...
    # قانون ۱۷ (دستور حمید ۱۳ سپتامبر): هر ۵ دقیقه صندوقِ ورودی + سطلِ
    # هر انجین، تا ایجنت‌ها به‌جای بازخوانیِ همه‌چیز فقط «تغییر» را بخوانند.
    dict(key="intake", every=300, wf="pump-radar.yml", timeout=150,
         desc="اسکن‌بردار — وضعیتِ قرارداد + خوراک بیرونی، هر ۵ دقیقه",
         cmd=["python3", "-m", "hamid.intake", "--write"]),
    dict(key="dispatch", every=300, wf="pump-radar.yml", timeout=60,
         desc="دسته‌بند — سطلِ هر انجین/ایجنت با read_next",
         cmd=["python3", "-m", "hamid.dispatch", "--write"]),
    ...
    dict(key="tg_audit", every=3600, wf="hamid-cycle.yml", timeout=120,
         desc="ممیزی ۷روزهٔ پیام‌های تلگرام + نتیجهٔ سیگنال‌های ارسالی",
         cmd=["python3", "-m", "hamid.tg_audit", "--write"]),
    dict(key="holding_intake", every=900, wf="hamid-cycle.yml", timeout=60,
         desc="صندوق هولدینگ — فایل‌های نشست/داده، بی‌پرسش (قانون ۱۸)",
         cmd=["python3", "-m", "hamid.holding_intake", "--write"]),
    dict(key="live_results", every=900, wf="hamid-cycle.yml", timeout=60,
         desc="نتایج لایو بیت‌یونیکس از فایل صادرشده + تطبیق با سیگنال",
         cmd=["python3", "-m", "hamid.live_results", "--write"]),
    dict(key="guardian_delta", every=1800, wf="hamid-cycle.yml", timeout=120,
         desc="دلتای رأی و دفتر ۱۲ مراقب",
         cmd=["python3", "-m", "hamid.guardian_delta", "--write"]),
    dict(key="handoff", every=1800, wf="hamid-cycle.yml", timeout=60,
         desc="نسخهٔ انتقال به چتِ تازه (HANDOFF.md)",
         cmd=["python3", "-m", "hamid.handoff", "--write"]),
    # چهار تولیدکننده‌ای که Actions می‌زد و جدول محلی جا انداخته بود
    # (ممیزی اتصال انجین‌ها، ۲۶ سپتامبر — test_liam9d سرخ بود).
    dict(key="history_room", every=1800, wf="hamid-cycle.yml", timeout=300,
         desc="اتاق تاریخچه — سابقهٔ هر ارز",
         cmd=["python3", "-m", "hamid.history_room", "--write"]),
    dict(key="market_stance", every=1800, wf="hamid-cycle.yml", timeout=300,
         desc="موضع بازار — نقشهٔ پایه، AT_POINT/WAIT",
         cmd=["python3", "-m", "hamid.market_stance", "--write"]),
    dict(key="macro_guard", every=900, wf="pump-radar.yml", timeout=120,
         desc="پنجرهٔ خبر کلان USD — سایز نصف، وتو نه",
         cmd=["python3", "-m", "hamid.macro_guard", "--write"]),
    dict(key="tf_geo_arms", every=28800, wf="work-report.yml", timeout=300,
         desc="داور سه بازوی تایم‌فریم/هندسه (قانون ۲۰)",
         cmd=["python3", "-m", "hamid.tf_geo_arms", "--write"]),
