...
def run():
    ...
    check("سررسید در آینده‌ای معقول است (حداکثر ۷ روز)",
          0 < (cfg["until"] - now) <= 7 * 86400000,
          f"{round((cfg['until']-now)/3600000,1)} ساعت مانده")
    ...
    check("ترجیح همین حالا فعال است", pref == cfg["prefer"] and note["active"])
