...
def _verdict(dg, mv, cs):
    ...
    pct = 100 * n_dg / len(dg)
    if pct < 90:
        # عمداً «نشتی» گفته نمی‌شود، ولی جهتِ خطا حالا معلوم است و باید
        # صریح نوشته شود (قانون عددِ درست، ۵ سپتامبر):
        #
        # تطبیق **دقیق** است — همان خطی که `digest_closed` می‌نویسد
        # بازسازی و در دفتر دنبال می‌شود. تطبیقِ دقیق تقریباً هرگز
        # اشتباه مثبت نمی‌دهد، ولی اگر یکی از فیلدهای `why` بین ثبت و
        # سنجش عوض شده باشد، هضمِ واقعی را جا می‌اندازد. پس این عدد
        # **کرانِ پایین** است.
        #
        # کرانِ بالا از کلیدِ ضعیفِ (ارز، جهت، R) می‌آید که نسخهٔ قبلی
        # داشت و ۲۸.۸٪ می‌داد — عددی که با شمارش باطل شد (۱۰۴۳ معاملهٔ
        # scalp «۲۸٪ تطبیق» گرفتند در حالی که دفتر تجربه صفر ردیف
        # strategy=scalp دارد). حقیقت جایی در همین بازه است.
        return (f"فقط {n_dg} از {len(dg)} معاملهٔ بسته در دفتر تجربه پیدا شد "
                f"({pct:.1f}٪) — با تطبیقِ **دقیقِ** همان خطی که "
                "`digest_closed` می‌نویسد. تطبیق دقیق اشتباه مثبت نمی‌دهد "
                "ولی می‌تواند جا بیندازد، پس این کرانِ پایین است؛ کرانِ "
                "بالا با کلیدِ ضعیفِ (ارز، جهت، R) ۲۸.۸٪ بود و آن کلید با "
                "شمارش باطل شد. هر دو سرِ بازه پایین است — ارزش ریشه‌یابی "
                "دارد.")
    if mv["status"] == "STUCK":
