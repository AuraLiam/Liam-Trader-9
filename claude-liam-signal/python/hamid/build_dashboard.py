...
نمی‌کنیم**؛ دو خروجی از یک منبع می‌سازیم:
  ...
  · `ghoghnoos.py` («ققنوس») — همان کد، بدون کامنت و داک‌استرینگ، برای
    جعبهٔ داشبورد. نام از دستور حمید (۱۱ سپتامبر)؛ فقط نامِ موتور است و
    امضای پنل روی خروجی همچنان «لیام تریدر ۹» می‌ماند.
...
# ── نام موتور داشبورد: «ققنوس» (دستور حمید، ۱۱ سپتامبر) ──────────────────
#
# تا امروز خروجی `liam9_strategy_dash.py` نام داشت. حمید نامش را ققنوس
# گذاشت — همان ایجنت اصلی (E00) که شورای دوازده مراقب زودیاک را می‌شنود
# و حکم نهایی را می‌دهد (قانون ۱۶).
#
# **این فقط نامِ موتور است.** سه چیز عمداً دست نخورد، چون هر کدام
# جای دیگری معنا دارد:
#   · امضای پنل روی خروجی («لیام تریدر ۹») — دستور ۱۶ اوت: حمید باید از
#     روی امضا بفهمد پیام از کدام پنل آمده. دو پنل، دو استراتژی.
#   · شناسهٔ استراتژی روی ردیف‌های دفتر — کارنامهٔ تاریخی به آن بسته است.
#   · `liam9_strategy.py` (منبعِ خوانا) — یک منبع حقیقت می‌ماند.
ENGINE_FA = "ققنوس"
OUT = PY / "ghoghnoos.py"
...
def banner(src, body):
    ...
    head = (
        "#!/usr/bin/env python3\n"
        f'"""{ENGINE_FA} — موتور استراتژی داشبورد «لیام تریدر ۹»'
        f" (نسخهٔ {v} فشرده)\n\n"
        f"نام موتور: **{ENGINE_FA}** — همان ایجنت اصلی که شورای دوازده\n"
        "مراقب زودیاک را می‌شنود و حکم نهایی را می‌دهد (قانون ۱۶).\n\n"
        ...
        "  · امضای پنل روی خروجی «لیام تریدر ۹» می‌ماند — نامِ موتور عوض\n"
        "    شد، برندِ ارسال نه (دستور ۱۶ اوت).\n"
        "  · LIVE_EXECUTION خاموش است؛ این موتور سیگنال و قصدِ اجرا\n"
        "    می‌سازد، خودش معامله باز نمی‌کند.\n"
        "  · هر پوزیشن ایزوله، با استاپ و تارگتِ اجباری (قرارداد ۲۰ اوت).\n\n"
