...
عملاً یکی است (۰.۳۵۹٪ در برابر ۰.۳۴۱٪):

| | n | خالص R | CI 95٪ |
|---|---|---|---|
| SHORT | ۶۸ | −۰.۱۹۴ | [−۰.۳۸۵, −۰.۰۰۴] |
| LONG | ۸۲ | −۰.۰۵۲ | [−۰.۲۲۱, +۰.۱۱۷] |
| **اختلاف (S−L)** | | **−۰.۱۴۲** | **[−۰.۳۹۷, +۰.۱۱۳]** — شامل صفر، ‎\|t\|=۱.۰۹ |

یعنی **هیچ تفاوت اثبات‌شده‌ای بین شورت و لانگ نداریم.**
...
ترکیبِ لانگ را به ترکیبِ شورت استاندارد می‌کنیم، خالصِ لانگ از −۰.۰۷۱
به **−۰.۱۰۲** می‌رود — یعنی حدود یک‌چهارمِ شکاف فقط ترکیب است، و بقیه‌اش
از نویز جدا نیست.

## علتِ واقعی که پیدا شد — هندسه، نه جهت

روی کل دفتر، بر حسب فاصلهٔ استاپ:

| فاصلهٔ استاپ | n | کارمزد | ناخالص | **خالص** |
|---|---|---|---|---|
| ۰–۰.۵٪ | ۱۱۵ | **۰.۳۰۳R** | +۰.۱۶۳ | **−۰.۱۴۰** |
| ۰.۵–۰.۸٪ | ۲۴ | ۰.۱۵۸R | −۰.۰۵۰ | −۰.۲۰۸ |
| **۰.۸–۱.۵٪** | ۲۹ | ۰.۰۹۲R | +۰.۱۱۷ | **+۰.۰۲۵** |
| >۱.۵٪ | ۶۷ | ۰.۰۳۴R | −۰.۰۶۶ | −۰.۱۰۰ |

دو بیماریِ جدا، در دو سر طیف:
- **استاپ تنگ**: لبهٔ ناخالص هست (+۰.۱۶۳) ولی کارمزد ۰.۳۰R می‌خورَدش.
  همان بیماریِ ثبت‌شدهٔ میز ۱ دقیقه و میز شوک — `کارمزد٪ ÷ استاپ٪`.
- **استاپ گشاد**: کارمزد بی‌اهمیت است ولی خودِ لبهٔ ناخالص منفی می‌شود
  (تارگتِ دور، مومنتوم خرج‌شده).

تنها باندی که خالصش مثبت است **۰.۸–۱.۵٪** است. این یافته به هیچ جهتی
گره نخورده و برای هر دو جهت صادق است.

## مرز

این فایل فقط می‌شمارد و بازه می‌سازد؛ هیچ دروازه‌ای را عوض نمی‌کند.
n باندِ ۰.۸–۱.۵٪ فقط ۲۹ است و CI‌اش صفر را در بر می‌گیرد — پس «فقط در
این باند معامله کن» هنوز یک **فرضیه** است نه قاعده، و ورودش به تصمیم
از مسیر قانون ۰۳ می‌گذرد: بک‌تست بیرون-از-نمونه روی کندل واقعی، CI
بالای صفر، و تأیید صریح حمید.
...
def main(argv=()):
    ...
    for b_lo, b_hi in STOP_BANDS:
        ...
        if len(g) < 5:
            continue
        fee = [r["fee_r"] for r in g if r.get("fee_r") is not None]
