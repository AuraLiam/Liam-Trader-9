...
def logged(kind, title, extra=None, msg_id=None):
    ...
    ۷ سپتامبر، دستور حمید («اون ۱۱ ماژول بی‌دفتر رو هم درست کن»): یازده
    ماژول به تلگرام می‌فرستادند و هیچ ردی نمی‌گذاشتند. یعنی هر شمارشی از
    «چند پیام رفت» کفِ واقعیت بود نه خودش. حالا همه از این‌جا رد می‌شوند.

    چرا تابعِ جدا و نه صدازدنِ مستقیمِ `record_out`: ثبت هرگز نباید
    ارسالی را که **قبلاً انجام شده** بترکاند. خطای دفتر چاپ می‌شود
    (بی‌صدا رد نمی‌شود، وگرنه همان نابینایی برمی‌گردد) ولی بالا نمی‌رود.
    """
    try:
        return record_out(kind, title, extra, msg_id=msg_id)
    except Exception as e:                           # noqa: BLE001
        print(f"telegram: دفترِ «{kind}» نوشته نشد ({type(e).__name__})",
              flush=True)
        return None
    ...
    for s in fresh:
        ...
        try:
            ...
            if not _ta["ok"]:
                ...
                #
                # تا امروز این دروازه — که بیشترین شورت را می‌کشد — هیچ
                # کارنامه‌ای نداشت: دفتر `vetoed` فقط `premortem` را ثبت
                # می‌کرد (۲٬۹۸۵ ردیف در برابر صفر). پس سؤالِ «وتویی که
                # این‌همه شورت را می‌کشد پول نجات می‌دهد یا می‌سوزاند؟»
                # بی‌جواب بود — نه چون جواب ندارد، چون اندازه گرفته
                # نمی‌شد. اصلِ «هر دروازه باید خودش نمره بگیرد» (قانون
                # ۰۳/۱۲) این‌جا اجرا نشده بود.
                #
                # این ردیف **هیچ رفتاری را عوض نمی‌کند**: وتو سرِ جایش
                # است و سیگنال نمی‌رود. فقط همان ستاپ در دفتر کاغذی باز
                # می‌شود تا `paper.mark` با کندل واقعی ببندَدش و ماشین
                # شبانه بتواند با CI بگوید وتو درست بوده یا نه.
                #
                # سیل نمی‌شود: خط ۹۷۵ ستاپی را که کلید `skip|` دارد از
                # همین ابتدا کنار می‌گذارد، پس هر ستاپ یکتا در پنجرهٔ
                # ضدتکرار فقط یک بار به این‌جا می‌رسد.
                #
                # مرحله‌اش `gate-vetoed` است و در `paper._NOT_SIGNAL`
                # نشسته — یعنی هرگز «سیگنال ارسالی» شمرده نمی‌شود و
                ...
                try:
                    ...
                    _gv_n = _paper.open_from(
                        [{"symbol": s["sym"], "dir": s["dir"],
                          ...
                          "tp2": s.get("tp2"), "stage_tag": "gate-vetoed",
                          "tf": s.get("tf")}],
                        {"veto_why": "trend_gate",
                         "gate_reason": str(_ta.get("reason") or "")[:120],
                         # سه فیلدی که برشِ شبانه با آن‌ها معنا پیدا می‌کند:
                         # وتوی مطلقِ دو-تایم در برابر تأییدِ ناقصِ یک-تایم،
                         # و اینکه کدام تایم مخالف بوده.
                         "trend_4h": _ta.get("t4"), "trend_1h": _ta.get("t1"),
                         "trend_mode": _ta.get("mode"),
